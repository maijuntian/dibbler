package com.mai.dibbler.widget;
...
public class SearchView extends EditText {
    ...
    private void DrawSearchIcon(Canvas canvas) {
        if (this.getText().toString().length() == 0) {
            float textWidth = paint.measureText("搜索课程名称");
            ...
            canvas.drawText("搜索课程名称", getScrollX() + searchSize + 8, getScrollY() + (getHeight() - (getHeight() - textHeight) / 2) - paint.getFontMetrics().bottom - dy, paint);
            canvas.restore();
        }
    }
    ...
    protected void onAttachedToWindow() {
        ...
        if (mDrawable == null) {
            try {
                mDrawable = getContext().getResources().getDrawable(R.mipmap.ydgzncf_icon_sousuo);
